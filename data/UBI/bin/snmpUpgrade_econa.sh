...
PROTOCOL=$5
DEVICENAME=$6
HW_VERSION=`/pfrm2.0/bin/printProductData /dev/mtd6 HwVer`
#FIRMWARE=`/pfrm2.0/bin/ShowFirmwareVerInt /dev/mtd2`
FIRMWARE_VERSION=`cat /var/firmwareVerInt`
TFTP_PROTOCOL="1"
FTP_PROTOCOL="2"
...
cp -fp /usr/sbin/flash_eraseall /mnt/tmpfs/ 
...
$UPGRADE_PROGRAM /tmp/$FILENAME $DEVICENAME $KERNEL_PARTITION $ROOTFS_PARTITION $FIRMWARE_VERSION $NEW_CONFIG_PARTITION $HW_VERSION 
echo "$UPGRADE_PROGRAM /tmp/$FILENAME $DEVICENAME $KERNEL_PARTITION $ROOTFS_PARTITION $FIRMWARE_VERSION $NEW_CONFIG_PARTITION $HW_VERSION" >>  /var/snmpFirmwareParams
