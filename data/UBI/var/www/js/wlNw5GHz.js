...
/****
 * cancel click
 * @method radioConfig5hzReset
 */
function radioConfig5hzReset(){
	this.tf1_frmradioSettings5GHz.reset();
	pageInit(); 
	wlNwInit ()	;
}
...
function updateChannelSpacing ()
    ...
    if (modeVal == 212992 || modeVal == 196608 || modeVal == 65536)
    	{
    	chSpIdx = 1;
    	chSpEndIdx = 1;
    	}
    ...
    /* remove n only(131072),na(393216),ac only(983041),a/n/ac(983040) if ElevenNDisabled =1 starts */
	if(wlNwObjLst[cntIdx].ElevenNDisabled == 1){
	 ...
	 /* remove n only(131072),na(393216),ac only(983041),a/n/ac(983040) if ElevenNDisabled =1 ends */
	 ...
	 then you need to hide 20/40 Mhz channel space for mode na and n only mode starts */
	 ...
	 /* if chSpSt 0 and chSpEnd 1 and chSp40LowNaChLst/chSp40UpNaChLst is empty 
	 then you need to hide 20/40 Mhz channel space for mode na and n only mode  ends */
    updateChannel ();	
	}
...
function updateMode ()
	...
	for (opIdx = 0; opIdx < mdLst.length; opIdx++)
    	...
    	/* Below code is written to remove empty options appending to select box starts */
    	  var selModeArray =[];
    	  var pushedValue = "";
    	  var toBePush ="";
...
function getCountryIndex ()
   ...
   var selectedCnt = $("#hdWlanCountry2").val();
      //  console.log("in2"+selectedCnt);
...
function updateOperationFrequency ()
    ...
    if(modelIdObj)
        ...
        if(modelIdObjVal == model || modelIdObjVal == model2 || modelIdObjVal == model3 || modelIdObjVal == model4)
...
function wlNwInit ()
	...
	selectRegion ();
    updateCountries ();
    optionValueSelect ('tf1_selCountry','hdWlanCountry2');
    updateOperationFrequency ();
...
function selectRegion ()
	{
	var cntCode = document.getElementById ('hdWlanCountry2').value;
...
function getCurrentChannel()
	...
	request.onreadystatechange=function()
        ...
        if (typeof radioCardNo === 'undefined'){
           radioCardNo = 1
        }
...
function checkRadiusPSK (rdbStationMode)
	{
	var wlanSecType = parseInt (radioCheckedValueGet (rdbStationMode), 10);
	var selectedValue = comboSelectedValueGet ('selRadiusPSK');
	if (!selectedValue) return;
	switch (parseInt (selectedValue, 10))
		{
		case 1:		/* RADIUS */
			fieldStateChangeWr ('','tblPSK','','tblRadius');
			switch (wlanSecType)
				{
				case 32:	/* WPA */
	  				fieldStateChangeWr ('rdbAES rdbTkipAES','','rdbTKIP','');
					fieldStateSet ('rdbTKIP','');
					break;
				case 64:	/* WPA2 */
	  				fieldStateChangeWr ('rdbTKIP rdbTkipAES','','rdbAES','');
					fieldStateSet ('rdbAES','');
					break;
				case 96:	/* WPA + WPA2 */
	  				fieldStateChangeWr ('rdbTKIP rdbAES','','rdbTkipAES','');
					fieldStateSet ('rdbTkipAES','');
					break;
				default:
					break;
				}
			break;
		default:	/* PSK */
			fieldStateChangeWr ('','tblRadius','','tblPSK');
			switch (wlanSecType)
				{
				case 32:	/* WPA */
	  				fieldStateChangeWr ('rdbTkipAES','','rdbAES rdbTKIP','');
					break;
				case 64:	/* WPA2 */
	  				fieldStateChangeWr ('rdbTKIP rdbTkipAES','','rdbAES','');
					fieldStateSet ('rdbAES','');
...
function wepKeyLenGet ()
	{
	var encrMethod = comboSelectedValueGet ('selWepEncrMethod');
	var keyLen = 0;
...
function generateWepKeys ()
	...
	var wepPsk = document.getElementById ('txtWepPsk').value;
	if ( wepPsk == "" )
		{
		document.getElementById('txtWepPskErr').innerHTML = "Enter Pass Phrase";
		return 
		}
	document.getElementById ('txtWepKey1').value = hex_md5 (wepPsk).substr (0, keyLen);
	wepPsk += document.getElementById ('txtWepKey1').value;
	document.getElementById ('txtWepKey2').value = hex_md5 (wepPsk).substr (0, keyLen);
	wepPsk += document.getElementById ('txtWepKey2').value;
	document.getElementById ('txtWepKey3').value = hex_md5 (wepPsk).substr (0, keyLen);
	wepPsk += document.getElementById ('txtWepKey3').value;
	document.getElementById ('txtWepKey4').value = hex_md5 (wepPsk).substr (0, keyLen);
	wepPsk += document.getElementById ('txtWepKey4').value;
	}	
function profileSecurityCheck ()
	{
	var encryptionList = ["TKIP", "CCMP", "TKIP+CCMP"];
	var selectedValue = comboSelectedValueGet  ('selSecurityOpt');
	if (!selectedValue) return;
	switch (selectedValue)
		{
		case 'WPA+WPA2':
			fieldStateChangeWr ('chkPreAuthStatus','tblWEP','selEncType selAuthType txtWPAPasswd','');
			break;
		case 'WPA2':
			fieldStateChangeWr ('chkPreAuthStatus','tblWEP','selEncType selAuthType txtWPAPasswd','');
            var selectedAuth = comboSelectedValueGet  ('selAuthType');
            if (selectedAuth == 'RADIUS')
                 fieldStateChangeWr ('','','chkPreAuthStatus', '');
			break;
		case 'WPA':
			fieldStateChangeWr ('chkPreAuthStatus','tblWEP','selEncType selAuthType txtWPAPasswd','');
			break;
		case 'WEP':
			fieldStateChangeWr ('selEncType selAuthType txtWPAPasswd chkPreAuthStatus','','','tblWEP');
			wepKeyIdCheck ();
			break;
		default:
			fieldStateChangeWr ('selEncType selAuthType txtWPAPasswd chkPreAuthStatus','tblWEP','','');
		...
		AuthTypeCheck ();
	var chObj = document.getElementById ('selEncType');
...
function profileConfigInit ()
	...
	optionValueSelect ('selEncType','hdEncType');
	enableCheck ();
	}
function AuthTypeCheck ()
	{
	var selectedValue = comboSelectedValueGet  ('selAuthType');
	if (!selectedValue) return;
	switch (selectedValue)
		{
		case 'PSK+RADIUS':
			fieldStateChangeWr ('chkPreAuthStatus','','txtWPAPasswd','');
			break;
		case 'RADIUS':
			fieldStateChangeWr ('txtWPAPasswd chkPreAuthStatus','','','');
            var selectedSec = comboSelectedValueGet  ('selSecurityOpt');
            if (selectedSec == 'WPA2')
                fieldStateChangeWr ('','','chkPreAuthStatus','');
			break;
		default:
...
function setWlInfo ()
	{
	var countryValObj = document.getElementById ('hdWlanCountry2');
	var countryObj = document.getElementById ('tf1_tdCountry');
	...
	resetChannelAndMode ();
	/*var chValObj = document.getElementById ('hdWlanChannel');
	var chObj = document.getElementById ('tdChannel');
	if (chValObj && chObj)
		{
		if (!isNaN (chValObj.value))
			chObj.innerHTML = chLst[parseInt (chValObj.value, 10)];
		}*/
	}
...
function pageInit ()
	...
	var cntryobj = document.getElementById ('hdWlanCountry2');
...
function changeTransmission(){ 
	...
	$("#selTransmitRate").html(transMissionRateOptions);
	   ...
	   if ($("#tf1_unitName").val() == "DSR-1000AC") {
		...
		transMissionRatesAC = [{'val':0,'label':'Best(Automatic)'},{'val':11,'label':6},{'val':15,'label':9},{'val':10,'label':12},{'val':14,'label':18},{'val':9,'label':24},{'val':13,'label':36},{'val':8,'label':48},{'val':12,'label':54},{'val':25,'label':5.5},{'val':26,'label':2},{'val':27,'label':1},{'val':128,'label':'MCS0'},{'val':129,'label':'MCS1'},{'val':130,'label':'MCS2'},{'val':131,'label':'MCS3'},{'val':132,'label':'MCS4'},{'val':133,'label':'MCS5'},{'val':134,'label':'MCS6'},{'val':135,'label':'MCS7'},{'val':136,'label':'MCS8'},{'val':137,'label':'MCS9'},{'val':138,'label':'MCS10'},{'val':139,'label':'MCS11'},{'val':140,'label':'MCS12'},{'val':141,'label':'MCS13'},{'val':142,'label':'MCS14'},{'val':143,'label':'MCS15'}];
	  }
		for (var i =0; i < transMissionRatesAC.length; i++) {
...
transMissionRatesAC =[{'val':0,'label':'Best(Automatic)'},{'val':128,'label':'MCS0'},{'val':129,'label':'MCS1'},{'val':130,'label':'MCS2'},{'val':131,'label':'MCS3'},{'val':132,'label':'MCS4'},{'val':133,'label':'MCS5'},{'val':134,'label':'MCS6'},{'val':135,'label':'MCS7'},{'val':136,'label':'MCS8'},{'val':137,'label':'MCS9'},{'val':138,'label':'MCS10'},{'val':139,'label':'MCS11'},{'val':140,'label':'MCS12'},{'val':141,'label':'MCS13'},{'val':142,'label':'MCS14'},{'val':143,'label':'MCS15'}];
	   ...
	   if ($("#tf1_unitName").val() == "DSR-1000AC") {
		...
		transMissionRatesAC =[{'val':0,'label':'Best(Automatic)'},{'val':11,'label':6},{'val':15,'label':9},{'val':10,'label':12},{'val':14,'label':18},{'val':9,'label':24},{'val':13,'label':36},{'val':8,'label':48},{'val':12,'label':54},{'val':128,'label':'MCS0'},{'val':129,'label':'MCS1'},{'val':130,'label':'MCS2'},{'val':131,'label':'MCS3'},{'val':132,'label':'MCS4'},{'val':133,'label':'MCS5'},{'val':134,'label':'MCS6'},{'val':135,'label':'MCS7'},{'val':136,'label':'MCS8'},{'val':137,'label':'MCS9'},{'val':138,'label':'MCS10'},{'val':139,'label':'MCS11'},{'val':140,'label':'MCS12'},{'val':141,'label':'MCS13'},{'val':142,'label':'MCS14'},{'val':143,'label':'MCS15'},{'val':144,'label':'MCS16'},{'val':145,'label':'MCS17'},{'val':146,'label':'MCS18'},{'val':147,'label':'MCS19'},{'val':148,'label':'MCS20'},{'val':149,'label':'MCS21'},{'val':151,'label':'MCS23'},{'val':152,'label':'VHT0'},{'val':153,'label':'VHT1'},{'val':154,'label':'VHT2'},{'val':155,'label':'VHT3'},{'val':156,'label':'VHT4'},{'val':157,'label':'VHT5'},{'val':158,'label':'VHT6'},{'val':159,'label':'VHT7'},{'val':160,'label':'VHT8'},{'val':161,'label':'VHT9'}];
