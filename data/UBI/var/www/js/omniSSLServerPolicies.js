 ...
 * OnLoad validation
 */
function omniSSLServerPolicyFn(){
 ...
 * @method omniSSLPoliciesValidate
 */
function omniSSLPoliciesValidate(frmId){
    ...
    if (checkMinMax() == false) 
        return false;
    var availUsersObj = document.getElementById("tf1_availUsers");
	if (availUsersObj && !availUsersObj.disabled){
		if (availUsersObj.selectedIndex == -1) {
			alert(LANG_LOCALE['50020']);
			return false;
		}
	}
    setHiddenChks(frmId);
...
function applyPolicyToType(selName){
    ...
    switch (parseInt(selValue, 10)) {
        case 1: /* IP Address */
            fieldStateChangeWr('tf1_txtMaskLen', '', 'tf1_txtIPAddress tf1_txtStrPort tf1_txtEndPort');
            vidualDisplay('tf1_txtMaskLen', 'hide');
            vidualDisplay('break6 break11', 'hide');
            vidualDisplay('tf1_txtIPAddress tf1_txtStrPort tf1_txtEndPort', 'configRow');
            vidualDisplay('break6 break7 break8 break10', 'break');
            break;
            
        case 2: /* IP Network */
            fieldStateChangeWr('', '', 'tf1_txtIPAddress tf1_txtMaskLen tf1_txtStrPort tf1_txtEndPort');
            vidualDisplay('tf1_txtIPAddress tf1_txtMaskLen tf1_txtStrPort tf1_txtEndPort', 'configRow');
            vidualDisplay('break6 break7 break8', 'break');
            break;
