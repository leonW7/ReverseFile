...
/* 
 * funcName    : convertIPToNUM
 * In Arg      : IP address in string format.
 * out Arg     : IP address in integer format.
 * Description : js function for aton
 */
function ipv4AsciitoNum (token)
    {
    ...
    /* calculate the number equivalent */
    result = ((octet0 & 0xff) << 24) |
    ((octet1 & 0xff) << 16) |
    ((octet2 & 0xff) << 8) |
    (octet3 & 0xff);
...
/* 
 * funcName    : convertNumToIP
 * In Arg      : IP address in integer format.
 * out Arg     : IP address in string format.
 * Description : js function for ntoa
 */
function ipv4NumtoAscii (token)
    {
    /* declare or define your variables here */
    /* return the converted IP address in string format */
    return ((token >> 24) & 255) + "." + ((token >> 16) & 255) + "." + ((token >> 8) & 255) + "." + (token & 255);
    }

/* 
 * funcName    : getIPv4Range.
 * In Arg      : IP address, subnetMask.
 * out Arg     : networkAddress, broadcastAddress, startIP, endIP. 
 * Description : Subnet range validator function.
 */ 
function getIPv4RangeAttr (ip, subnet) 
   {
   /* Declare or define your variables here */
   var networkAddr;
   var networkAddrStr;
   var broadcastAddr;
   var broadcastAddrStr;
   var startAddrStr;
   var endAddrStr;
   var ipv4AddrAttributes = new Array();

   /* calculate the network address */
   networkAddr = ip & subnet;
   networkAddrStr = ipv4NumtoAscii (networkAddr);
   
   /* calculate the broadcast address */
   broadcastAddr = (ip | (~subnet));
   broadcastAddrStr = ipv4NumtoAscii (broadcastAddr);
   
   /* calculate the start address */
   startAddrStr = ipv4NumtoAscii ((networkAddr + 1));

   /* calculate the end IP address */
   endAddrStr = ipv4NumtoAscii ((broadcastAddr - 1));

   /* copy all the contents to this array */
   ipv4AddrAttributes[0] = networkAddrStr;
   ipv4AddrAttributes[1] = broadcastAddrStr;
   ipv4AddrAttributes[2] = startAddrStr;
   ipv4AddrAttributes[3] = endAddrStr;

   /* return the array back to callee */
   return ipv4AddrAttributes;
   }
...
function ipRangeValidate(ipAddrStartId, ipAddrEndId){
    ...
    if (errorFlag) {
        var errorStr = LANG_LOCALE['10924'];
        alert(errorStr)
        endIpObj.focus();
        return false;
    }
    ...
    for (var i = 0; i < objArr.length; i++) {
        ...
        if (alertFlag) {
            ...
            if ((ipv4AddrTypeStr == 'SM' || ipv4AddrTypeStr == 'SNM') && ((i + 1) == 1)) 
                errMsg += LANG_LOCALE['30420'];
            else 
                if ((ipv4AddrTypeStr == 'SM') && ((i + 1) == 4)) {
                    if (objArr[i - 1] != "255") 
                        errMsg += LANG_LOCALE['30421'];
                    else 
                        errMsg += LANG_LOCALE['11963'];
                }
                else 
                    if ((ipv4AddrTypeStr == 'SM' || ipv4AddrTypeStr == 'SNM') && ((i + 1) > 1)) {
                        if (objArr[i - 1] != "255") 
                            ...
                            errMsg += LANG_LOCALE['11964'];
                    }
                    else {
                        errMsg += LANG_LOCALE['12162'] + ipOctetMinVal +
                        " - " +
                        ipOctetMaxVal +
                        " ";
                    }
            ...
            if (errMsg) {
                if ((i == 0) && (j == objArr.length) && errMsg) {
                    if (ipv4AddrTypeStr == 'SM' || ipv4AddrTypeStr == 'SNM') 
                        if (alertFlag) {
                            errMsg = prefixStr + LANG_LOCALE['12161'];
                            alert(errMsg);
                        }
                        else 
                            if (alertFlag) {
                                errMsg = prefixStr + LANG_LOCALE['12162'] + ipOctetMinVal + " - " + ipOctetMaxVal + LANG_LOCALE['11029'];
                                alert(errMsg);
                            }
                }
                else {
        ...
        /* we allow CIDR notations 
         * NSN - New Subnet for CIDR.
        */
        if (((i + 1) == 4) && (ipv4AddrTypeStr == 'NSN')) {
            /* last octet can set to zero as this is network ip */
            ipOctetMinVal = 0;
            ipOctetMaxVal = 255;
        }
        if (((i + 1) == 1) && (ipv4AddrTypeStr == 'IP' || ipv4AddrTypeStr == 'SN' || ipv4AddrTypeStr == 'NSN')) {
                ...
                if ((ipv4AddrTypeStr == 'SM' || ipv4AddrTypeStr == 'SNM') && ((i + 1) == 1)) 
                    ...
                    if ((ipv4AddrTypeStr == 'SM') && ((i + 1) == 4)) {
                        ...
                        if ((ipv4AddrTypeStr == 'SM' || ipv4AddrTypeStr == 'SNM') && ((i + 1) > 1)) {
                            ...
                            errMsg += LANG_LOCALE['12162'] + ipOctetMinVal + " - " + ipOctetMaxVal + " ";
                        }
                if (suffixStr) 
                    errMsg += suffixStr;
                if (octetShowFlag) 
...
var Validate = {};
/*
 * Validate.subnetValidate - validates for subnet. Subnet configured should not be 
 * configured in the subnet of LAN/VLAN
 * @param -  ipAddr1 - ip address field id
 * @param -  subnetId1 - ip address subnet
 * @param -  ipAddr2Pre - prefix Id for vlan IP Address hidden fields
 * @param -  ipAddr2Pre - prefix Id for vlan subnet mask hidden fields
 * @param -  hdLengthId - id representing number of vlans fields
 * @param -  msg - message to be shown 
 *returns true/false;  
 */ 
   Validate.subnetValidate = function ( ipAddr1, subnetId1, ipAddr2Pre, subnetId2Pre, hdLengthId, msg ) {
   var hdLengthIdObj = document.getElementById (hdLengthId);
   for ( var i = 1; i <= parseInt ( hdLengthIdObj.value, 10 ); i++ ) {
       if (subnetValidationWr ( ipAddr1, subnetId1, ipAddr2Pre + i, subnetId2Pre + i )) {
           alert (msg);
           document.getElementById(ipAddr1).focus();
           return false;
           }
       }
       return true;
...
/*
Function: ipV4FormatCheck
Purpose: checks for ipv4 format
Returns: false or true
*/  
function ipV4FormatCheck (ipAddr)
    {
...
/*
 * Function: hostNameValidation
 * Checks: Host Name validation
 * Returns: true or false
 * */
function hostNameValidation (fieldId) {
    var regExp = /^(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z0-9]|[A-Za-z0-9][A-Za-z0-9\-]*[A-Za-z0-9])$/g;
    var hostNameObj = document.getElementById(fieldId);
    if (hostNameObj && !hostNameObj.disabled && hostNameObj.value != '') {
    var status = regExp.test(hostNameObj.value);
        if (!status) {
            alert(LANG_LOCALE['50052']);
            hostNameObj.focus();
            return status;
        }
    }
